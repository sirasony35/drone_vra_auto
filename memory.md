...
- **2026-09-09 화성 HSR 변량시비 효과 분석 리포트** (`analyze_vra_effect.py` 신규 커밋, 결과 `result/hs_vra_effect_report/`):
  - 데이터: `data/hs_data`(시비前 07-15, HSR1~14 중 11필지, 드론 GNDVI 2cm) + `data/hs_data/after_vra`(시비後 07-28) + 처방맵 `result/hs_0720`. 변량/관행 구분은 `after_vra/화성변량시비여부.xlsx`(HS-R1형식→HSR1). **변량 7(HSR1·2·6·7·11·13·14, spread1.2) / 관행 4(HSR3·4·8·12, spread0)** — xlsx와 hs_vra.csv spread 일치.
  - 방법: 작물 마스크(GNDVI 0.2~1.0) 후 **균일도=CV(변동계수)** 의 前後 변화 비교. 지표 CV·평균·P10(저위)·저활력<0.5%. before/after는 13일 간격 독립 분포로 비교(픽셀 정합 불필요, CV는 평균정규화라 성장 보정됨).
  - **결과: 변량 평균 ΔCV -0.84%p(개선) vs 관행 +1.31%p(악화)**, 저위 P10 회복 변량 +0.036 vs 관행 +0.012 → **변량이 관행보다 균일화·저위회복 우수**(그룹차 ~2.15%p). 대표 성공 HSR11(CV16.6→13.2, 저활력11.2→3.6%, 처방맵이 저위부에 증량). 예외: HSR6·7(변량인데 CV↑+평균GNDVI↓=생육저하 의심), HSR8·12(관행인데 성숙으로 개선).
  - **유의(리포트 명시)**: 13일 성숙에 따른 GNDVI 상승·상단포화 공존 → 절대값 아닌 그룹'차이'로 판단 / 표본 소수(관행4) / 변량 필지가 시비 총량도 더 많음(60kg vs 20~60kg)=총량효과 일부 기여. 통계검정(t-test)·HSR6·7 원인분석은 미실시(요청 시).
  - 리포트 13p PDF(커버+요약차트+필지별 11장: 좌 시비前·중 처방맵·우 시비後 GNDVI + 前後 히스토그램 + 지표표). PDF/PNG는 result/(gitignore), 스크립트만 커밋.

- **2026-08-19 송주(SM13~24)·이천 완료 처방 + 이천 통합 리포트** *(직전 세션 작업, memory 기록 누락분 보완)*:
  - **송주(sj)**: `data/드론변량시비_송주`(SM13~24 12필지, 드론 GSD 2.1cm) + 새 경계 zip(`_바운더리/SMxx_Boundary_new.zip`). **SM13~18=400kg(20포)·SM19~24=500kg(25포)**, DJI 1m 변량. `vra_setting/sj_vra.csv`(+`sj_xag_vra.csv` XAG판 5m 3등급). 결과 `result/sj_dji_0819`·`result/sj_xag_0819`. 커버리지 99%(드론). 총 5,400kg.
  - **이천 완료**: `data/경기도이천_완료` 49필지를 `이천필지.xlsx` 경작자·비료와 리(지역) 단위로 매칭해 `icheon_dji_vra.csv`에 추가(활성50+완료49=**99필지**, 경작자 7명). 김민섭·이한범·조현수 400평당20kg / 이치의 1000평당20kg(조현수 '다뿌림'→변량 확정). 완료 처방 `result/icheon_wanryo_0819`.
  - **이천 통합 리포트**(활성+완료 99필지, 경작자별): `result/icheon_report/이천_변량시비_결과보고서.pdf`(107p). 화성 리포트와 동일 포맷(경작자별 종합 7장 + 필지별 상세 99장). 생성 스크립트는 스크래치패드·미커밋. **버그교훈**: 오버뷰 좌표를 `transform_bounds`(left,bottom,right,top)로 받아 imshow extent(left,right,bottom,top)에 잘못 매핑하면 세계지도로 깨짐 — 순서 정렬 필수.
  - `002d5a9 "setting commit"`으로 vra_setting 전체 CSV+xlsx가 git 추적됨(이전 미추적 유실위험 해소).

- **2026-08-25 간척지 시연용(GCJDEMO) 변량 처방맵 — 합성 GNDVI 기반**:
  - 요청: `data/간척지_데모`의 경계 shp(zip)만으로 **3포(60kg)·DJI·1m 격자·살포량 차이 극명한** 시연용 처방맵.
  - **전제 확인 결과 GNDVI 영상이 없었음**: 데모 경계(1,716평=0.567ha, `name=간척지비료테스트`)는 lon 126.68698~126.68817인데 기존 GCJ 필지 중 최동단 GCJ1-3이 126.68654에서 끝나 **약 40m 벗어남** — `data/` 전체 tif 어디에도 안 잡힘. 사용자 확인 후 **시연용 합성 패턴**으로 진행 결정(생육 근거 아님, 기체 동작·살포량 변화 시연 전용).
  - **`demo_gndvi_generator.py` 신규**: 경계만 있고 영상이 없을 때 쓰는 합성 GNDVI 생성기. 필지 주축을 따라 5개 띠로 나눠 `BAND_VALUES=[0.33,0.47,0.60,0.68,0.75]`를 `BAND_ORDER=[2,0,4,1,3]`로 **섞어 배치**(기체가 비행하며 살포량이 여러 번 바뀌도록), 띠 내부 잡음 σ0.008, 경계 밖 nodata=-10000, CRS는 기존 간척지와 같은 EPSG:32652. 사용법 `python demo_gndvi_generator.py <경계zip|shp> <출력tif> [GSD]`. 파일 상단에 **'실제 촬영 데이터 아님' 경고 명시**.
  - 실행: 합성 tif(GSD 0.2m) → `vra_setting/gancheokji_demo_vra.csv`(GCJDEMO, total 60, **spread 2.0**, grid 1m, DJI, overage 1.0) → `result/gancheokji_demo_0825`. 경계 zip은 `find_boundary_zip`이 첫 토큰 매칭이라 **`data/간척지_데모_boundary/GCJDEMO.zip`으로 복사**해 필지코드와 일치시킴.
  - 결과: 등급별 **192 / 136 / 88 / 62 / 49 kg/ha (최고÷최저 3.92배)**, 총량 60.00kg 정확, 커버리지 98.7%, Rx 픽셀도 5개 값으로 딱 떨어짐. **spread 2.0 = "50~200kg/ha 극대 대비"에 정확히 대응** — 향후 시연 요청 시 이 값 재사용.
  - **`overview_map.py` 줌 상한 버그 수정**: 소필지 1개만 그리면 자동 줌이 Esri 제공 한계를 넘어 'Map data not yet available' 회색 타일이 깔림 → `MAX_ZOOM=18` 상한 추가(5필지 개요는 화질 변화 없음).

- **2026-08-23 간척지(GCJ) 5필지 DJI 처방 + 전체 개요 지도 도구 신설** *(집 PC 작업. 이 기록은 08-19 회사 PC memory.md를 pull하며 한 번 유실돼 08-25에 복원함 — 회사 PC의 08-19 GCJ2-2·2-3 처방과는 별개 작업)*:
  - 입력 `data/간척지` GCJ1-1·1-2·1-3·2-2·2-3 5필지(260812, GNDVI만, EPSG:32652 **GSD 4.66cm=드론**). 경계 파일 없어 `data/간척지_boundary` 빈 폴더 + footprint 자동 감지. 격자 1m·`overage_factor=1.0`(격자≥GSD 규칙 충족, 드론이라 보정 불필요).
  - `vra_setting/gancheokji_vra.csv`(15컬럼): 전 필지 **DJI·1m·절대량 180kg(9포)**, field_area는 footprint 실측 — 5,542 / 5,211 / 5,041 / 5,003 / 5,038평(합 25,836평=8.54ha). 결과 `result/gancheokji_0823_dji`.
  - 5필지 전량 성공, 총 900kg(45포), **커버리지 98.2~98.6%(평균 98.4%)로 역대 최고**(SM 5m 93.5%·GR 1m 95.6% 대비). 평균살포량 99.6~110.4kg/ha.
  - **`overview_map.py` 신규**(프로젝트 루트): 결과 폴더의 DJI Rx GeoTIFF들을 **위성 배경지도(Esri World Imagery) 위에 한 장으로** 합쳐 개요 PNG 생성. 필지 경계선·라벨(필지명/총량·포대수/평)·등급 범례·전체 합계 표기. 사용법 `python overview_map.py [결과폴더] [출력파일명]`.
    - 구현 요점: Rx에는 등급이 아니라 kg/ha가 저장돼 있어 **VRA.csv의 등급별 Rate와 최근접 매칭으로 등급을 역산**(필지마다 rate가 달라 필지별 매핑). 배경 타일과 좌표계를 맞추려 Rx를 EPSG:3857로 재투영 후 `imshow(extent=...)`. contextily 실패 시 배경 없이 그리고 경고만 출력.
    - `contextily` 1.7.1 설치(python312 env), `requirements.txt`에 선택 의존성으로 주석과 함께 추가 — 처방 파이프라인 본체·exe엔 불필요.
  - **필요포대수 10 표기**: GCJ1-1·1-3은 총량이 180.004kg으로 떨어져 올림 때문에 10포로 표시(실질 9포). GJR16·SM과 동일한 기존 표시 특성.
  - ※ **memory.md 유실 주의**: 집 PC에서 memory.md를 고친 뒤 커밋하지 않으면 회사 PC 기록을 pull할 때 사라질 수 있음(이번에 실제 발생). 집 PC 작업 후에는 memory.md도 커밋할 것.

- **2026-08-19 화성 서비스 필지 결과보고서 생성** (`result/hs_service_report/`, 스크립트는 스크래치패드에만 있음·미커밋):
  - 입력: `vra_setting/화성_서비스필지.xlsx`(지역·경작자·주소·면적·비료량). **39지번 → 실제 촬영 22필지**(한 영상이 여러 지번 포함, 예 `장안리 3059,3060,3061`). 3경작자 — **이현우 6·이현지 5·신성용 11필지**. `리+지번`으로 촬영 필지↔경작자 전량 매칭.
  - **① 경작자별 종합 3장**(`경작자별/`): **Esri 위성영상 배경**(contextily pip 설치, EPSG:3857) + 각 필지 GNDVI 레이어(다운샘플 imshow) + 청록 외곽박스 + 노랑 번호마커 + **우측 주소 목록(번호 매칭)**. 필지 좌표는 GNDVI(5179) bounds를 3857로 transform해 배치. 밀집 필지(이현우)는 GNDVI 선명, 넓게 퍼진 필지(신성용)는 작게 보임.
  - **② 필지별 상세 22장**(`필지별/`): 제목=필지주소(경작자 병기), **좌 GNDVI(생육, 녹색 cmap)·우 처방맵(kg/ha, RdYlGn_r)**, 하단 등급표(**등급 / 실면적(평·ha) / ha당 처방(kg/ha) / 실면적 처방량(kg·포)** + 합계).
  - **③ 통합 PDF 26p**(`화성_서비스_결과보고서.pdf`, PIL로 A4가로 흰 캔버스 맞춤): 커버 + 경작자별[종합1+상세N] 순.
  - **주의 — 처방은 기존 `result/hs_service_0721`(일괄 1500평당 3포) 그대로 사용**. xlsx 경작자별 세팅(이현우 1200평당 3.5포=70kg, 이현지 1500평당 5포=100kg, 신성용 1500평에 3포=60kg)과 **불일치**(사용자가 "기존 처방 그대로" 선택). 보고서 처방량도 3포 기준. 경작자별로 맞추려면 vra.csv 재작성 후 재처방 필요. GNDVI는 `data/hs_service_data`(EPSG:5179, ~2cm 드론). 보고서 렌더 스크립트(hs_detail/hs_overview/hs_pdf)는 미커밋이라 재현 시 재작성.

- **2026-08-19 간척지(GCJ) 2필지 처방 — 드론 4.7cm GSD, 1m 격자 적합**: `data/간척지`(GCJ2-2·GCJ2-3, GNDVI EPSG:32652 **GSD 4.7cm**=드론)에 경계 zip(`data/간척지/Boundary/*_Boundary.zip`) 제공. **필지당 180kg(9포) 절대량·DJI·1m·rice**, 변량. `vra_setting/gcj_vra.csv`, 결과 `result/gcj_dji_0818`. 드론 GSD라 1m 격자에 셀당 ~450픽셀 → 커버리지 97~98%, **보정계수 불필요**(overage_factor 비움). 각 180kg 정확, GNDVI 0.58~0.83(건강). 필지코드=`GCJ2-2`(첫 `_`토큰, 충돌 없음). "격자≥GSD" 규칙 재확인 사례(위성 새만금 1.5m와 대비 — 드론이면 1m OK).

- **2026-08-18 살포 보정계수(overage_factor) 기능 + 격자·GSD 기준 확립** (커밋 `228d281`, exe 재빌드 반영):
  - **핵심 발견 — 격자는 데이터 GSD 이상이어야 한다**: 새만금7공구는 **위성 데이터(GSD 1.5m/픽셀)**. 격자를 1m로 낮추니(데이터보다 촘촘) 셀의 절반이 무효화 → 커버리지 44%로 붕괴, 살포량 500~819kg/ha, 등급 1·5 소멸, 얼룩. **드론 데이터는 GSD ≈2cm**(함평/괴산 실측 0.019~0.020m)라 1m 격자가 이상적(2500픽셀/셀)이지만, 위성은 붕괴. **규칙: 격자 ≥ GSD.** SM01 스윕 실측 — 격자 1m=44%/1.5m=80%/**2m=96%(최적)**/3m=94%/5m=94% 커버리지. 위성·대규모는 **2m 권장**.
  - **현장 오차 = 커버리지 부족**: 실제 살포 ≈ 처방 ÷ 커버리지. 실측(SM11·12 5m→60포/1200kg=처방900의 ×1.33, SM10 2m→50포/1000kg=×1.11). 즉 커버리지↑ = 오차↓. 1m(44%)이면 오차 최대(~×2), "촘촘=정확"은 GSD까지만 성립.
  - **`overage_factor` 컬럼 신설**(vra.csv 15번째): `_resolve_total_kg`에서 **목표 ÷ overage_factor = Rx 처방**으로 낮춤(절대량·비율 모드 공통). 드론이 ×배율 과살포해도 실제가 목표에 수렴. 컬럼 없음/1.0=보정 없음(하위호환). **기준: 드론(2cm)·육지 소규모=1.0 / 위성(1.5m)·대규모=실측배율(2m 1.11, 5m 1.33)**. 배율은 필지 모양(길쭉할수록↑)에 의존하니 필지군별 실측 재보정 권장.
  - **요약엑셀 변경**: `보정계수`·`Rx처방적분(kg)` 컬럼 추가. `총비료량(kg)`=**적재기준(예상 실제=목표)**, `Rx처방적분`=지도 적분값(=VRA.csv 등급합). 예: 새만금 각 필지 총비료 900kg(45포)/Rx 811kg. Result.png 정보줄도 보정 시 `Rx→예상실제` 표기. factor=1.0이면 두 값 동일(기존과 같음).
  - **필지코드 정규화**: 새만금7공구 원본 파일명 `SM01_A-12_..._GNDVI.tif`는 첫 `_`토큰이 `SM01`로 잘려 CSV(`SM01(A-12)`)·경계shp와 불일치 → `data/sm7_dji_run/`에 `SM01(A-12)_...`로 복사·정규화해 처리(원본 보존). 경계는 `data/ShapeFile`의 측량 shp 사용. SM13~23은 CSV 없어 제외.
  - **범례에 면적(평) 통합**(같은 세션, 커밋 `3e95a7b`+병합 `a9d7691`): 집 PC의 rate_map 범례(kg/ha·실제kg)와 회사 PC의 면적(평) 범례가 pull 충돌 → 통합판으로 해결. 최종 범례 = `1(High)  155평 · 389kg/ha · 20.0kg (1.0포)`, 제목 `등급별 면적 · 살포량(kg/ha) · 총비료`. **pull 충돌로 operation_main.py 삭제됐던 것 복구**(양쪽 save_map_image 각자 구현이 원인).
